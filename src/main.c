...
#define LED_COLOR_KEEP			0x0F		// Keep the current settings
...
#define	LED_PATTERN_KEEP		0x0F		// Keep the current settings

// Number of buzzers
#define	BUZZER_COUNT_CONTINUE	0x00		// Continuous operation
#define	BUZZER_COUNT_KEEP		0x0F		// Keep the current settings
...
#define	BUZZER_KEEP				0x0F		// Keep the current settings

// Buzzer volume
#define	BUZZER_VOLUME_OFF		0x00		// Mute
#define	BUZZER_VOLUME_MAX		0x0A		// Maximum volume
#define	BUZZER_VOLUME_KEEP		0x0F		// Keep the current settings
...
int SendCommand(char* sendData, int sendLength)
{
	...
	if (devHandle == NULL) {
		puts("device handle is not null");
		return -1;
	}
...
/// Get touch sensor input status
/// </summary>
/// <returns>Acquisition failure:-1, Touch sensor input OFF:0、Touch sensor input ON:1</returns>
int GetTouchSensorState()
{
	int ret;
    unsigned char sendData[SEND_BUFFER_SIZE];
